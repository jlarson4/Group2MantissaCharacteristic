...
bool add(int c1, int n1, int d1, int c2, int n2, int d2, char result[], int len) {
	...
	//if the numerator is negative but the number is positive
	if (return_numerator < 0 && return_number > 0) {
		return_number--;
		return_numerator = return_numerator + return_denominator;
	}
	//if numerator is positive but number is negative
	else if (return_numerator > 0 && return_number < 0) {
		return_number++;
		return_numerator = -return_numerator + return_denominator;
	}
	//if numerator is negative and the number is negative
	else if (return_numerator < 0 && return_number < 0) {
		return_numerator *= -1;
	}
	...
	//if the numerator is negative and the number is 0

	if (return_number == 0 && return_numerator < 0) {
		return_numerator *= -1;
		result[0] = '-';
		index = 1;
	}
...
int add_mantissa_to_array(int n, int d, char result[], int len, int index) {
	...
	char* decimal = convert_mantissa_to_char(d, n, space);
...
char * convert_mantissa_to_char(int return_denominator, int return_numerator, int space) {
	char * decimal_nums = new char[space];
	for (int i = 0; i < space; i++) {
		if (return_numerator != 0) {
